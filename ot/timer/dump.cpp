...
void Timer::_dump_slack(std::ostream& os) const {
  ...
  if(!_pins.empty())  {
    ...
    for(const auto& kvp : _pins) {
      ...
      FOR_EACH_EL_RF(el, rf) {
        os << std::setw(10);
        if(auto slack = pin.slack(el, rf); slack) os << (*slack).get_constant();
        else os << "n/a";
        os << "  ";
      }
...
void Timer::_dump_at(std::ostream& os) const {
  ...
  if(!_pins.empty())  {
    ...
    for(const auto& kvp : _pins) {
      ...
      FOR_EACH_EL_RF(el, rf) {
        os << std::setw(10);
        if(auto at = pin.at(el, rf); at) os << (*at).get_constant();
        else os << "n/a";
        os << "  ";
      }
...
void Timer::_dump_rat(std::ostream& os) const {
  ...
  if(!_pins.empty())  {
    ...
    for(const auto& kvp : _pins) {
      ...
      FOR_EACH_EL_RF(el, rf) {
        os << std::setw(10);
        if(auto rat = pin.rat(el, rf); rat) os << (*rat).get_constant();
        else os << "n/a";
        os << "  ";
      }
