...
// Constructor
Path::Path(Statisical_delay slk, const Endpoint* ept) :
  slack    {slk},
  endpoint {ept} {
}
...
void Path::dump(std::ostream& os) const {
  ...
  for(const auto& p : *this) {
    ...
    // else os << p.at;
    // yclo
    if(pi_at) os << p.at.mean() - (*pi_at).mean();
    else os << p.at.mean();
    
    // arrival time
    // os << std::setw(w3) << p.at;
    // yclo
    os << std::setw(w3) << p.at.mean();
  ...
  std::visit(Functors{
    [&] (Test* test) {
      ...
      OT_LOGW_IF(
        ...
        "unstable numerics in PBA and GBA rats: ", sum, " vs ", rat.mean()
      );
    },
    [&] (PrimaryOutput* po) {
      os << std::setw(w1) << "port";
